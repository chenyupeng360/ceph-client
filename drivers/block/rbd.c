...
static int rbd_img_fill_from_bio(struct rbd_img_request *img_req,
				 struct ceph_file_extent *img_extents,
				 u32 num_img_extents, struct bio *bio)
{
	struct ceph_bio_iter it = { .bio = bio, .iter = bio->bi_iter };

	return __rbd_img_fill_from_bio(img_req, img_extents, num_img_extents,
				       &it);
}

static int rbd_img_fill_from_bio_nodata(struct rbd_img_request *img_req,
					struct ceph_file_extent *img_extents,
					u32 num_img_extents,
					struct bio *bio)
{
	...
	rbd_assert(!bio_has_data(bio));
	return rbd_img_fill_request(img_req, img_extents, num_img_extents,
				    &fctx);
}

static void set_pos(struct ceph_object_extent *ex, u32 bytes, void *arg)
{
	struct rbd_obj_request *obj_req =
	    container_of(ex, struct rbd_obj_request, ex);
...
static void rbd_queue_workfn(struct work_struct *work)
{
	...
	struct ceph_file_extent ex = { offset, length };
	enum obj_operation_type op_type;
	u64 mapping_size;
	bool must_be_locked;
	int result;
	...
	if (op_type == OBJ_OP_DISCARD)
		result = rbd_img_fill_from_bio_nodata(img_request, &ex, 1,
						      rq->bio);
	else
		result = rbd_img_fill_from_bio(img_request, &ex, 1, rq->bio);
	if (result)
